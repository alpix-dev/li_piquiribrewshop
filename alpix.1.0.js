...
theme.lang.productListAdd = "Comprar";
...
theme.build.header = function(template){
    ...
    $('#theme_header-functions').prepend('<li><button type="button" class="account-trigger">'+ theme.icon.account +'<div><b>Minha Conta</b>Olá, <span>'+ theme.userFirstname +'</span></div></button></li>');
...
theme.build.footer = function(template){
    ...
    $('#theme_footer-pages').append(theme.footerPages);
    //$('#theme_footer-content2').append(theme.footerCategories);
    $('#theme_footer-payments').append(theme.footerPayments);
    $('#theme_footer-payments').append(theme.footerGateways);
    $('#theme_footer-seals').append(theme.footerSeals);
    $('#theme_footer-institutional').append(theme.storeDescription);
    $('#theme_footer-social').append(theme.socialIcons);
    ...
    $('#theme_footer-contact').prepend('<div class="contact"><i class="fa fa-envelope-o email"></i><div><b>Email</b><a href="mailto:contato@piquiribrewshop.com.br" target="_blank">contato@piquiribrewshop.com.br</a></div></div>');
    $('#theme_footer-contact').prepend('<div class="contact"><i class="fa fa-clock-o atendimento"></i><div><b>Atendimento</b> Segunda à Sexta, das 09 às 12h - 13h30 às 18h<br>Sábado das 9h às 12h</div></div>');
    if(theme.storeWhatsapp){
        $('#theme_footer-contact').prepend('<div class="contact whatsapp"><i class="fa fa-whatsapp"></i><div><b>WhatsApp</b><a href="https://wa.me/55'+ (theme.storeWhatsapp.replaceAll('(','').replaceAll(')','').replaceAll('-','').replaceAll(' ','')) +'" target="_blank"><span>Dúvidas ou auxílio em seu pedido</span>'+ theme.storeWhatsapp +'</a></div></div>');    
    }    
    if(theme.storePhone){
        $('#theme_footer-contact .fa-clock-o + div').append('<div style="margin-top:5px"><a href="tel:'+ (theme.storePhone.replaceAll('(','').replaceAll(')','').replaceAll('-','').replaceAll(' ','')) +'" target="_blank">'+ theme.storePhone +'</a></div>');    
    }

    // $('<li><a href="#"><div class="cover"><img src="https://via.placeholder.com/140x140"/></div><div class="content"><h5>Conheça a Kveik!</h5><p>Kveik é uma palavra norueguesa que significa levedura. na trdição norueguesa, a Kveik era preservada por secagem e passada de geração</p></div></a></li>').appendTo('.theme_footer-blog');
    // $('<li><a href="#"><div class="cover"><img src="https://via.placeholder.com/140x140"/></div><div class="content"><h5>5 Fundamentos da BOA fermentação</h5><p>Leveduras saudáveis: a mais importante da fermentação é a levedura, pois, ela converte açúcar em álcool, dióxido de carbono e outros...</p></div></a></li>').appendTo('.theme_footer-blog')
...
theme.build.search = function(template){
    ...
    $('#theme_search, .theme_search').append(theme.searchForm);

    $('#theme_search #auto-complete, .theme_search #auto-complete').attr('placeholder',theme.lang.searchPlaceholder);
    $('#theme_search .botao.icon-search, .theme_search .botao.icon-search').text(theme.lang.searchButtonText);
    $('#theme_search .botao.icon-search, .theme_search .botao.icon-search').removeClass('icon-search');

    $('.search-trigger').click(function(){   
        $('body').toggleClass('asideSearch-visible');         
        $('#theme_search input, .theme_search input').val('');
        //$('#theme_search input').autocomplete('destroy');
    });

    $("#theme_search input, .theme_search input").autocomplete({
        delay: 300,
        minLength: 5,
        source: function(o, n) {
            $.ajax({
    ...
    $("#theme_search input, .theme_search input").each(function() {
...
theme.functions = [];
// theme.functions.onlyFullbanner = function(){
//     	$('.secao-banners .span9').toggleClass('span9 span12');
//         $('.secao-banners .span3').hide();
// };
   ...
   flag = $('.secao-banners img').length;
    $('.secao-banners img').each(function(k, i){
        ...
        if(trigger.includes('[banner-texto]')){
            ...
            if(description.includes('/')){
                ...
                $.each(description,function(k,v){
                    if(k != 1){
                        ...
                        html_description.append("<strong style='"+ (cor_focus ? "color:" + cor_focus + ";" : '') +"'>"+ v +"</strong>");
                    }
            ...
            if(txt_btn){
                html_content.append("<button type='button' style='"+ (cor_focus ? "color:" + cor_focus + ";" : '') +"'>"+ txt_btn +"</button>");
            }
...
theme.functions.createField = function (oObj){
    ...
    if("text,number,password,tel,email,date,color".split(',').includes(oObj.type)){
        field.append('<input type="'+ oObj.type +'" name="theme_field-'+ oObj.id +'" required="'+ oObj.required +'"/>');
    }
    if(oObj.type == "textarea"){
        field.append('<textarea rows="4" name="theme_field-'+ oObj.id +'"></textarea>');
    }
    if(oObj.type == "select"){
        let select = $('<select name="theme_field-'+ oObj.id +'" required="'+ oObj.required +'"></select>');
...
theme.functions.init = function(){
    ...
    theme.icon.cart = '<img src="https://cdn.awsli.com.br/431/'+ window.LOJA_ID +'/arquivos/ico_cart.png"/>';
    ...
    theme.icon.account = '<img src="https://cdn.awsli.com.br/431/'+ window.LOJA_ID +'/arquivos/ico_account.png"/>';
    ...
    if (!theme.templates.header) theme.templates.header = '<div id="theme_header_1"> <div class="conteiner-fluid"> <div class="row-flex align-items-center"> <div class="col-auto" id="theme_header-logo"> <button type="button" id="theme_header-menu-trigger"></button> </div><div class="col theme_search"></div><div class="col-auto"> <ul id="theme_header-functions"></ul> </div></div><div class="bg"> <div class="row"> <div class="col-12 justify-content-center" id="theme_header-menu"></div></div></div></div></div>';
    ...
    theme.templates.footer = '<div class="bg1"> <div class="conteiner"> <div class="row-fluid"> <div class="span5"> <h4>Sobre nós</h4> <div id="theme_footer-institutional"></div><div id="theme_footer-contact"></div></div><div class="span7"> <div class="row-flex justify-content-between align-items-start"> <div class="col-auto"> <h4>Institucional</h4> <div id="theme_footer-pages"></div><div id="theme_footer-social"></div></div><div class="col-auto"> <h4>Últimas do Blog</h4> <ul class="theme_footer-blog"></ul> </div></div></div></div></div></div><div class="bg2"> <div class="conteiner"> <div class="row-fluid"> <div class="span7"> <h4>Pague com</h4> <div id="theme_footer-payments"></div></div><div class="span5"> <h4>Selos</h4> <div id="theme_footer-seals"></div></div></div></div></div>';
...
theme.functions.resizeBanners = function(){
    ...
    $('.modulo img').each(function(){
        let url = $(this).attr('src').replace('400x400/','660x660/').replace('1140x1140/','1920x1920/');
        $(this).attr('src',url);
...
theme.functions.productListImageSize = function(param){
    ...
    $('.listagem-item .nome-produto').equalHeights();
...
theme.functions['pagina-produto'] = function(){
    ...
    $('.abas-custom #descricao').closest('.abas-custom').before('<div class="listagem"><h4 class="titulo cor-secundaria">Descrição</h4></div>');
